...
export default function BillPreview() {
  ...
  const { orderToClose } = location.state || {}
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      setConfirmError('Couldn\'t close order — try again. Make sure you have internet.')
    } finally {
      setConfirming(false)
    }
  }
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-100 px-4 py-3 flex items-center gap-3">
        ...
        <h2 className="flex-1 text-center font-semibold text-gray-900">Close Order</h2>
        <div className="w-7" />
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 pt-4 pb-32">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-5 font-mono text-sm mb-5 shadow-sm">
          ...
          {(table_number || customer_name) && (
            <div className="text-xs text-center mb-3 text-gray-600 font-medium">
          ...
          <div className="border-t border-dashed border-gray-300 pt-3 space-y-2">
            {order_items.map((item, i) => (
              <div key={i} className="flex items-start justify-between gap-2 text-xs">
                <span className="text-gray-700 flex-1">{item.menu_items?.name || 'Item'}</span>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-4 py-4 safe-area-pb">
        ...
        >
          {confirming ? 'Closing Order…' : 'Close Order & Generate Bill'}
        </button>
      </div>
    </div>
  )
